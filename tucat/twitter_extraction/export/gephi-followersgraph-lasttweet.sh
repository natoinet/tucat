...
mongoexport --quiet --host $MONGOHOST --db $DBNAME --collection $2 --fields $MONGO_FIELDS_NODE --query "$QUERY" --csv -o "$OUTPUT_FILES/archivo-node-$2-$COLLECTION.csv"
...
mongoexport --quiet --host $MONGOHOST --db $DBNAME --collection $COLLECTION --fields $MONGO_FIELDS_EDGE --csv -o "$OUTPUT_FILES/archivo-edge-$2-$COLLECTION.csv"
...
mongo --quiet --eval --host $MONGOHOST "db.getSiblingDB('$DBNAME')['$COLLECTION'].drop()"

#echo "Finished> $OUTPUT_FILES/grafo-followers-$1-$2-$COLLECTION.gdf - Thank you for your patience."
echo "grafo-followers-$2-$LTDATE-$COLLECTION.gdf"
