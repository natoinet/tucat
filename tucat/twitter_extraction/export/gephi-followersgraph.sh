...
mongoexport --quiet --host $MONGOHOST --db $DBNAME --collection $COLNAME --fields $MONGO_FIELDS_NODE --query "$QUERY" --csv -o "$OUTPUT_FILES/archivo-node-$COLNAME-$COLLECTION.csv"
...
mongoexport --quiet --host $MONGOHOST --db $DBNAME --collection $COLLECTION --fields $MONGO_FIELDS_EDGE --csv -o "$OUTPUT_FILES/archivo-edge-$COLNAME-$COLLECTION.csv"
...
echo "$OUTPUT_FILES/grafo-followers-$COLNAME-$COLLECTION.gdf"
